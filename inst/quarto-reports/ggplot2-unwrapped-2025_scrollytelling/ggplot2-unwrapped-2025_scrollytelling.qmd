...
format:
  closeread-html:
    ...
    narrative-text-color-sidebar: "#F9F7F1"
    section-background-color: red;
    include-in-header: 
      text: |
        <script src="https://kit.fontawesome.com/2c8252c536.js" crossorigin="anonymous"></script>
echo: false
warning: false
message: false
from: markdown+emoji
...
if (params$executed_from == "direct_from_file") {
  load(system.file("quarto-reports", "ggplot2-unwrapped-2025_scrollytelling","data_example_geom_usage.RData", package = "ggplot2wrapped"))

  data_geom_usage <- data_example_geom_usage
...
data_summary_geom_calls <- data_geom_usage |> 
  ggplot2wrapped::summarise_ggplot2_geom_usage()
...
data_total_times_used_top_n <- data_summary_geom_calls |> 
  ggplot2wrapped::summarise_top_4_ggplot_geom_usage(summarise_by = "geoms")

data_used_across_files_top_n <- data_summary_geom_calls |> 
  ggplot2wrapped::summarise_top_4_ggplot_geom_usage(summarise_by = "files")
...
::::: {style="--cr-narrative-font-size:var(--size-large);--cr-narrative-sidebar-width: 1fr;--cr-section-background-color:#AA3D4F;"}

::::{.cr-section layout="sidebar-right"}

::: {#cr-welcome .large-fixed-text}
<span style='color:#F9F7F1;'>Welcome to your year in {ggplot2!}</span>

![](images/logo.png){width="250px"}

> <span style='color:#F9F7F1;'>Interactive report built with love by {ggplot2wrapped} from Charlotte Hadley at GPCDS</span>
:::
...
Let it chart! 
<br/>
Let it dataviz!]{.medim-fixed-text style='color:#F9F7F1;'}
[<i class="fa-solid fa-arrow-down fa-6x"></i>]{style='color:#F9F7F1;'}
...
::: {#cr-shout-out-most-common-geom .large-fixed-text}
<span style='color:#F9F7F1;'>Shout out to `{r} most_used_geom_total_usage`</span>
:::

@cr-shout-out-most-common-geom

[That was your most used geom in 2025! Let's look at your other top geoms!]{.medim-fixed-text style='color:#F9F7F1;'}
...
#| fig.height: 6
data_geom_usage |> 
  ggplot2wrapped::make_geom_usage_pies_highcharts(pie_measure_type = "geom_total_usage")
```
:::
...
#| fig.height: 6
data_geom_usage |> 
  ggplot2wrapped::make_geom_usage_pies_highcharts(pie_measure_type = "geom_usage_in_files")
```
:::

<!---- START: calendars ---->
::::: {style="--cr-narrative-font-size:var(--size-large);--cr-narrative-sidebar-width: 2fr;--cr-section-background-color:#F9F7F1"}
...
<br>
In total `{r} scales::label_number(big.mark = ",")(sum(data_summary_files_per_day$total_files))` files were created that contained geoms!
...
You got your ggplot2 thesaurus out on:
<br/>
<span style="font-size:var(--size-normal)">`{r} paste(date_most_unique_geoms[["modified_date"]], collapse = ", ")`</span>
<br/>
On these dates you used `{r} unique(date_most_unique_geoms[["calendar_measure"]])` unique geoms!
<br>
<span style="font-size:var(--size-normal)">`{r} stringr::str_wrap(vec_geoms_from_most_unique_day)`</span>
::::

:::::

::::: {style="--cr-narrative-font-size:var(--size-large);--cr-narrative-sidebar-width: 0.5fr;--cr-section-background-color:#F9F7F1"}
::::{.cr-section layout="sidebar-left"}
...
::::: {style="--cr-narrative-font-size:var(--size-large);--cr-narrative-sidebar-width: 0.5fr;--cr-section-background-color:#AA3D4F;"}

::::{.cr-section layout="sidebar-right"}

::: {#cr-thats-a-complicated-question .large-fixed-text}
<span style='color:#F9F7F1;'>Which geoms did you use together most often?</span>
:::

@cr-thats-a-complicated-question

[Well, that's complicated! Let's get into it!]{.medim-fixed-text style='color:#F9F7F1;'}
...
> Geoms only appear if they occur more than 10 times in your data and an interaction has to occur more than once

### Oh, wow! Tell me more about UpSet Charts

It's more than likely you've not seen many (or any!) UpSet Charts before. They were only invented in 2012, and the authors quite rightly ask for the [original paper](https://doi.org.10.1109/TVCG.2014.2346248, 2014) to be cited whenever they're used.

> Alexander Lex, Nils Gehlenborg, Hendrik Strobelt, Romain Vuillemot, Hanspeter Pfister. UpSet: Visualization of Intersecting Sets IEEE Transactions on Visualization and Computer Graphics (InfoVis), 20(12): 1983--1992, [doi:10.1109/TVCG.2014.2346248](https://doi.org.10.1109/TVCG.2014.2346248, 2014), 2014.
...
The 2025 edition of the {ggplot2wrapped} report doesn't focus on comparing package usage. 

But if you take a look at our [documentation](https://charliejhadley.github.io/ggplot2wrapped/) you can analyse that data yourself 

::::
:::::

::::: {style="--cr-narrative-font-size:var(--size-large);--cr-section-background-color:#F9F7F1"}
...
::: {#cr-look-at-arguments .large-fixed-text}
<span style='color:#F9F7F1;'>Let's wrap up looking at the arguments!</span>
:::

@cr-look-at-arguments

[This might be the last pun?]{.medim-fixed-text style='color:#F9F7F1;'}
...
There's a lot more you can do with the [{ggplot2} package](https://charliejhadley.github.io/ggplot2wrapped/) but we'll sign off with what you were probably expecting and we'd hate to let you down:
...
::: {#cr-sound-off .large-fixed-text}
<span style='color:#F9F7F1;'>That was fun! Next year there'll be more, this package and report was thrown together in a week</span>
:::

@cr-sound-off

[How was this built?]{.medim-fixed-text style='color:#F9F7F1;'}
...
- The report itself was written using a Quarto extension called [Closeread](https://closeread.dev/) for scrollytelling

> The report was built by me - Charlotte Hadley from [gpcds.com](www.gpcds.com). 
>
> I can be [found on LinkedIn](https://www.linkedin.com/in/charliejhadley/) if that's your thing.
>
> I would genuinely love to see you share your {ggplot2wrapped} reports and charts. 
>
> There's **lots** more you can do yourself with the `add_geom_usage_to_files()` function. Share what you build and consider contributing to the package, [https://github.com/charliejhadley/ggplot2wrapped](https://github.com/charliejhadley/ggplot2wrapped).

\

::: {.column-screen style="background-image:url(images/gpcds-wide-banner.png); background-size: 100vw; height: 396px; background-position: center +0%; background-repeat: no-repeat;"}
:::
